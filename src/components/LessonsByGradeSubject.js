...
const LessonsByGradeSubject = () => {
    ...
    const [lessonsByGradeSubject, setLessonsByGradeSubject] = useState([]);
    const [currentPage, setCurrentPage] = useState(0);
    const [pageCount, setPageCount] = useState(0);
    const [isLoading, setIsLoading] = useState(false);

    const loadLessonsByGradeSubject = async () => {
        const response = await fetch(`http://127.0.0.1:8000/api/lessonByGradeSubject/${grade_id}/${id}?page=${currentPage}`);
        const resData = await response.json();
        setLessonsByGradeSubject(resData.data);
            ...
            <section id={"searchByGrade"}
                ...
                <div className="relative">
                    ...
                    <div className={`grid grid-cols-4 gap-10 mt-5`}>
                        {
                            lessonsByGradeSubject.length > 0 && lessonsByGradeSubject.map((lesson) => <LessonCard
                                imgWidth={"w-full"}
                                imgHeight={"h-[182px]"}
                                key={lesson.id}
                                id={lesson.id}
...
export default LessonsByGradeSubject;